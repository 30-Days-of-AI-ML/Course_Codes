#This note book is plotting Indias Covid 19 data statewise manner
...
import numpy as np #deals with numerical datas
...
import folium #support pacakge in py : Open source machine learning library to have lil prediction 

#Derriving a eqaution for data plotting & Data preprocessing
india_dataset = pd.read_csv("/path") #Input your data over here
...
## Loading the dataset
india_date_state_wise_cases = india_dataset.groupby(['Date','State/UnionTerritory'])['Cured','Deaths','Confirmed'].sum().reset_index() #Reading the change by sum()
...
india_date_state_wise_cases['Mortality Rate(per 100)'] = (india_date_state_wise_cases['Deaths']/india_date_state_wise_cases['Confirmed'])*100 #Extracting the equation
